...
func getReportDataSetRawData(w http.ResponseWriter, r *http.Request){
  ...
  var record []string
    ...
    record = append(record, "ItemID")
    record = append(record, "ItemName")
    ...
    for i := 0; i < len(data.Rows); i++ {
     ...
     var record []string
      ...
      record = append(record, resultMap["ItemID"].(string))
      record = append(record, resultMap["ItemName"].(string))
      record = append(record, resultMap["Code"].(string))
      writer.Write(record)
    }
