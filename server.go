  ...
  var record []string
    ...
    for i := 0; i < len(data.Rows); i++ {
     ...
     var record []string
      ...
      record = append(record, resultMap["Notes"].(string))
    ...
    w.Header().Set("Content-Type", "text/csv") // setting the content type header to text/csv
	 ...
	 w.Header().Set("Content-Disposition", "attachment;filename=Sheet1.csv")
	 w.Write(b.Bytes())
}


func getReportCSVDataSetRawData(w http.ResponseWriter, r *http.Request){
    fmt.Println("URL: " + r.URL.Host + r.URL.Path)
    data := getReportCSVDataResultSet(w, r)
    columndata := getReportRawDataColumnSet(w, r)
    fmt.Println("Endpoint Hit: getReportCSVDataResultSet")

    b := &bytes.Buffer{} // creates IO Writer  
	 writer := csv.NewWriter(b) 

   var record []string
    record = append(record, "HQ_HQID")
    record = append(record, "CallHQ_ID")
    record = append(record, "Zone")
    record = append(record, "Region")
    record = append(record, "Territory_Owner")
    record = append(record, "Type")
    record = append(record, "FirstCall")
    record = append(record, "LastCall")
    record = append(record, "HQ_Name")
    record = append(record, "HQStatus")
    record = append(record, "HQNumber")
    record = append(record, "Address")
    record = append(record, "City")
    record = append(record, "State")
    record = append(record, "Zip")
    record = append(record, "PrimaryWholesaler")
    record = append(record, "PrimaryWholesalerCustNumber")
    record = append(record, "Frequency")
    record = append(record, "Year")
    record = append(record, "Week")
    record = append(record, "Quarter")
    ...
    record = append(record, "CallType")
    record = append(record, "LastDistributionCall")
    record = append(record, "Territory Coverage")
    record = append(record, "Notes")
    record = append(record, "Contact")
    for x := 0; x < len(columndata.Rows); x++ {
       ...
       record = append(record, resultColMap["ColumnName"].(string)) 
    } 
    ...
    for i := 0; i < len(data.Rows); i++ {
     ...
     var record []string
      ...
      record = append(record, resultMap["Quarter"].(string))
      record = append(record, resultMap["Assigned"].(string))
      record = append(record, resultMap["Call"].(string))
      record = append(record, resultMap["CallType"].(string))
      record = append(record, resultMap["LastDistributionCall"].(string))
      record = append(record, resultMap["TerritoryCoverage"].(string))
      record = append(record, resultMap["Notes"].(string))
      ...
      for x := 0; x < len(columndata.Rows); x++ {
         ...
         resultColInterface := resultReflectionColValue.Interface()
         resultColMap := resultColInterface.(map[string]interface{}) 
         record = append(record, resultMap[resultColMap["ItemID"].(string)].(string))
       } 
...
func getReportCSVDataResultSet(w http.ResponseWriter, r *http.Request) CSVDataSearchResults {
    r.ParseForm()
   ...
   for index, element := range f.Rules {
     ...
     if  (element.Data != ""){
       ...
       if(index > 0){
         filterBuffer.WriteString(" " + f.GroupOp + " ")
       }
    ...
    return dalGetCSVDataSet(requestedPageNumber, rowsCount, columnToSort, sortDirection, filterBuffer.String());
}

func getReportRawDataResultSet(w http.ResponseWriter, r *http.Request) CSVDataSearchResults{
    r.ParseForm()
    requestedPageNumber := r.FormValue("page")
    rowsCount :=  r.FormValue("rows")
    columnToSort :=  r.FormValue("sidx")
...
func getReportRawDataColumnSet(w http.ResponseWriter, r *http.Request) RawDataColumnResults {
    r.ParseForm()
    requestedPageNumber := r.FormValue("page")
    rowsCount :=  r.FormValue("rows")
    columnToSort :=  r.FormValue("sidx")
   ...
   for index, element := range f.Rules {
    ...
    return dalGetRawColumnsSet(requestedPageNumber, rowsCount, columnToSort, sortDirection, filterBuffer.String());
...
func handleRequests(serverAddress string) {
    ...
    myRouter.HandleFunc("/reportdatasetrawdata", getReportCSVDataSetRawData).Methods("GET", "POST")
